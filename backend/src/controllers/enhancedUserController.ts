...
import { users, metrics, userAchievements, achievements, salesGoals, leaderboardRankings } from '../models/schema'
import { eq, like, and, desc, sum, count, sql } from 'drizzle-orm'
...
export const enhancedUserController = {
  ...
  async getUserById(req: Request, res: Response) {
    try {
      ...
      res.json(createResponse(true, 'User retrieved successfully', { user: userWithMetrics }))
    } catch (error) {
  ...
  async updateUser(req: Request, res: Response) {
    try {
      ...
      const { password, passwordHash, email, ...safeUpdateData } = updateData
  ...
  async updateAvatar(req: Request, res: Response) {
    try {
      ...
      const { passwordHash, ...userResponse } = updatedUser
      res.json(createResponse(true, 'Avatar updated successfully', { user: userResponse }))
    } catch (error) {
      logger.error('Update avatar error:', error)
      res.status(500).json(createResponse(false, 'Failed to update avatar'))
    }
  },
  ...
  async getUserAnalytics(req: Request, res: Response) {
    try {
      ...
      res.json(createResponse(true, 'User analytics retrieved successfully', {
        user: user[0],
        period: days,
        analytics,
      ...
      res.status(500).json(createResponse(false, 'Failed to get user analytics'))
    }
  }
}